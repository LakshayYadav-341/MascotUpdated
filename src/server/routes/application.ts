        ...
        try {
            ...
            if (!file) {
                return res
                    ...
                    .send(
                        handler.error(
                            handler.fieldInvalid(
                                "resume",
                                "Alumni Request requires a document to upload type pdf."
                            )
                        )
                    );
            }
            ...
            const fileName = Hash.create(user + "--" + originalName.join("."))
                .replace(/\//g, "--");
            ...
            if (!url) {
                return res
                    .status(422)
                    .send(
                        handler.error("Error downloading file. Please upload the correct file")
                    );
            }
            ...
            if (!jobApplication) {
                return res
                    .status(404)
                    .json(handler.error(handler.STATUS_404));
            }
            ...
            return res
                .status(500)
                .json(handler.error("Internal Server Error"));
        }
    }
);
